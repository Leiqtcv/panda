...
figure(1)
kk= 0;
X10 = round(10*randn(n,1)/5)*5;
X30 = round(30*randn(n,1)/5)*5;
X50 = round(50*randn(n,1)/5)*5;
Y10 = X10+randn(size(X10));
Y30 = X30+randn(size(X30));
Y50 = X50+randn(size(X50));
g = 1-[7 7 7].^2./[10 30 50].^2
flag = 1;
kk = 0;
if flag
for ii = 1:n
	...
	cla
	cntr = -100:100;
	ps	= normpdf(cntr,0,10);
	ps = 50*ps/max(ps);
	Xp = [0 0 ps];
	Yp = [0 max(cntr) cntr];
	h = patch(Xp,Yp,'k'); set(h,'FaceColor',[.7 .7 .7]);
	hold on
	plot(1:ii,X10(1:ii),'k.');
	xlim([1 n]);
	ylim([-90 90]);
	box off
	axis off
	h = pa_horline(0,'k-'); set(h,'LineWidth',2);
	...
	axis square;
	pa_unityline('k-');
	h = pa_regline([0 g(1)]','k-'); set(h,'LineWidth',2);
axis off
	
	subplot(323)
	cla
	cntr = -100:100;
	ps	= normpdf(cntr,0,30);
	ps = 50*ps/max(ps);
	Xp = [0 0 ps];
	Yp = [0 max(cntr) cntr];
	h = patch(Xp,Yp,'k'); set(h,'FaceColor',[.7 .7 .7]);
	hold on
	plot(1:ii,X30(1:ii),'k.');
	xlim([1 n]);
	ylim([-90 90]);
	box off
	axis off
	...
	axis square;
	pa_unityline('k-');
	h = pa_regline([0 g(2)]','k-'); set(h,'LineWidth',2);
axis off

	subplot(325)
	cla
	cntr = -200:200;
	ps	= normpdf(cntr,0,50);
	ps = 50*ps/max(ps);
	Xp = [0 0 ps];
	Yp = [0 max(cntr) cntr];
	h = patch(Xp,Yp,'k'); set(h,'FaceColor',[.7 .7 .7]);
	hold on
	plot(1:ii,X50(1:ii),'k.');
	xlim([1 n]);
	ylim([-90 90]);
	box off
	axis off
	...
	axis square;
	pa_unityline('k-');
	h = pa_regline([0 g(3)]','k-'); set(h,'LineWidth',2);
...
for ii = [1:1:ngain ngain:-1:1]
	x = X(1:100);
	y = Y(1:100);
	err = sqrt((gain(ii)*y-x).^2);
	
	subplot(221)
	cla
	x2 = [(1:100)' (1:00)'];
	y2 = [X(1:100)';gain(ii)*Y(1:100)']';
% 	plot(x2,y2,'k-','Color',[.7 .7 .7]);
	hold on
	

	
	plot(1:100,x,'k.-','Color',[.7 .7 .7],'MarkerFaceColor',[.7 .7 .7],'LineWidth',1);
	plot(1:100,gain(ii)*y,'k.','MarkerFaceColor','k','LineWidth',1,'Color','k');
	ylim([-60 60]);
	xlim([1 100]);
	box off
	axis off
	...
	plot(X(1:100),Y(1:100),'k.','MarkerFaceColor',[.7 .7 .7],'LineWidth',1,'Color',[.7 .7 .7]);
	hold on
	plot(X(1:100),gain(ii)*Y(1:100),'k.','MarkerFaceColor','k','LineWidth',1,'Color','k');
	...
	N = smooth(N);
	N = N';
% 	barh(cntr,N*4,1,'FaceColor',[.7 .7 .7]);
	Xp = [0 0 N*8];
	Yp = [0 max(cntr) cntr];
	h = patch(Xp,Yp,'k');
	set(h,'FaceColor',[.7 .7 .7]);
	hold on
	plot(1:100,err,'k.','MarkerFaceColor','k','LineWidth',1,'Color','k');
	xlim([1 100]);
	ylim([0 60]);
	box off
	axis off

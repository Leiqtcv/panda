...
for fIdx = 1:nfiles
	...
	ntrials		= size(btn,2);
	BTN			= NaN(ntrials,1);
	dall		= diff(btn(:)); % difference/velocity/gradient (V)
	thresh = 0.2;
	for ii		= 1:ntrials
		d		= 1000*[0;diff(btn(:,ii))]; % difference/velocity/gradient (V)
		
		sel		= d>thresh; % anything greater than 2*std is considered as button press
	...
	RTLED	= [RTLED; rtled];
	INT		= [INT; ledint];
	idx = find(strcmp(str,fname(4:5)));
	SUB		= [SUB;repmat(idx,size(dt))];
...
for ii = 1:nper
	for jj = 1:nint
		for kk = 1:ndt
			...
			N(ii,jj,kk)	= sum(sel);
			if uper(ii)==0
				mu(ii,jj,kk)	= 1./nanmedian(1./(RT(sel)))-(udt(kk));
			else
				mu(ii,jj,kk)	= 1./nanmedian(1./RT(sel));
			end
...
x = repmat(uper,1,nint);


subplot(1,5,ii)
h = errorbar(x(2:end,:),mux(2:end,:),sdx(2:end,:),'o-');
hold on
errorbar(x(1,:),mux(1,:),sdx(1,:),'o-');
axis square
box off
ylim([100 600]);
xlim([-100 600]);
set(gca,'XTick',round(uper));
legend(h,num2str(uint),'Location','Best');
xlabel('Modulation period (ms)');
ylabel('Time re sound (ms)');
pa_horline(mux(1,1),'b:');
pa_horline(mux(1,2),'r:');
end
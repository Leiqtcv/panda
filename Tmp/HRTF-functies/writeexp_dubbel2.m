function writeexp_dubbel2(expfile,datdir,theta,phi,id,int,ledon,sndon,dummy,nrdtrials)
...
%         for k       = 1:nint
            ...
            if theta(i)==0
                ...
                fprintf(fid,'%s\t%d\t%d\t \t%d\t%d\t%d\t%d\t%d\n','Sky',0,2,100,0,0,1,ledon(i));%(i,j,k));
            end
            ...
            fprintf(fid,'%s\t%d\t%d\t%d\t%d\t%d\t%d\n',...
                'Snd1',theta(i),phi(i),id(i,1),int(i),1,ledon(i)+sndon(i));
%                 'Snd1',theta(i,j,k),phi(i,j,k),id(i,j,k),int(i,j,k),1,ledon(i,j,k)+sndon(i,j,k));
            % Sound2 on 
            fprintf(fid,'%s\t%d\t%d\t%d\t%d\t%d\t%d\n',...
                'Snd2',theta(i),phi(i)-1,id(i,2),int(i),1,ledon(i)+sndon(i));

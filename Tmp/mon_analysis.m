...
for ii = 1:n
	...
	box off
	drawnow
	xlabel('Stimulus \epsilon');
	ylabel('Response \epsilon');
	b = regstats(R,T,'linear','beta');
	h = pa_regline(b.beta,'k-'); set(h,'LineWidth',2);
	if b.beta(1)<0
		plsmn = '-';
	...
	str = [upper(fnames{ii}(1:2)) ',' stim ': \epsilon_T = ' num2str(b.beta(2),2) '\epsilon_R' plsmn num2str(abs(b.beta(1)),2)];
	title(str)
	...
	plot(N,T,'-','Color',[.7 .7 .7]);
	hold on
scatter(N,R,10,I,'filled');
	str = [upper(fnames{ii}(1:2)) ',' stim ];
	title(str)

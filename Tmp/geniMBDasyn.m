function [S3,M3,Fs] = geniMBDasyn(Lvl)
...
%% Creating signal stimulus + maskers; asynchronous iMBD
...
delaym21 = 100;								%adding delay for masker m21
delaym22 = delaym21 + N + 200;				%INCREASING DELAYS SCHOULD BE SPACED BY N + previous delay
delaym23 = delaym22 + N + 300;
delaym24 = delaym23 + N + 400;
...
S3 = zeros(1,round(nrep*nperiod)+delaym24); %creating sound
S3(1:N) = s3;								%filling first stimulus interval
S3(N+delaym21:2*N+delaym21-1) = m21;		%filling in first masker (m21) interval
S3(N+delaym22:2*N+delaym22-1) = m22;
S3(N+delaym23:2*N+delaym23-1) = m23;
S3(N+delaym24:2*N+delaym24-1) = m24;

M3 = zeros(1,round(nrep*nperiod)+delaym24);
M3(N+delaym21:2*N+delaym21-1) = m21;		%filling in first masker (m21) interval
M3(N+delaym22:2*N+delaym22-1) = m22;
M3(N+delaym23:2*N+delaym23-1) = m23;
M3(N+delaym24:2*N+delaym24-1) = m24;

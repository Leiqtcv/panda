...
for ii = 1:nChains
	...
	initsStruct(ii).delta		= delta;
end

%% RUN THE CHAINS
parameters		= {'b','a','sigma','beta','delta','wn'}; % The parameter(s) to be monitored.
...
for idx = 1:numSavedSteps
	hpred   = beta(idx)*gampdf(x-delta(idx),a(idx),b(idx));
	ypred       = conv(X,hpred);
	ypred       = ypred(1:N);

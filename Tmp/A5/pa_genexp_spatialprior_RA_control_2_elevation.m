function pa_genexp_spatialprior_RA_control_2_elevation
...
maxled      = 800;
snd			= 8;
expfile     = ['spatialprior_A5_control_2_elevation.exp'];
datdir      = 'DAT';
minsnd      = 200;
maxsnd      = 200;
...
el = az;
for ii = 1:n
	az(ii) = 5*round(sdaz(ii)*randn(1)/5);
	el(ii) = 5*round(sdel(ii)*randn(1)/5);
	while abs(az(ii))+abs(el(ii))>90 || el(ii)<-55 || el(ii)>85 || abs(az(ii))>90 || (abs(az(ii))+abs(el(ii)))<3
	az(ii) = 5*round(sdaz(ii)*randn(1)/5);
	el(ii) = 5*round(sdel(ii)*randn(1)/5);
	end

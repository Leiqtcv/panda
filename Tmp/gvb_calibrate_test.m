function varargout = gvb_calibrate_test(varargin)
...
cd('D:\Dropbox\PhD\Research\Experiments\Results')
calfile = 'GB-GB-2014-04-10-0000';
load(calfile);
tar		= StimulusLocation;
taraz	= tar(:,1);
tarel	= tar(:,2);
res = HeadMovement;
mures = mean(res,2);
mures1 = squeeze(mures(:,1,1));
mures2 = squeeze(mures(:,1,2));
mures3 = squeeze(mures(:,1,3));
handles.NhiddenHor = 5;
handles.NhiddenVer = 5;
handles.H = mures2';
handles.V = mures3';
handles.F = mures1';
handles.Fix = 1:length(mures1);
handles.TarAz = taraz;
handles.TarEl = tarel;
% 
% lv                                  = length(varargin);
...
% set(handles.edit_nhidden_ver,'String',num2str(handles.NhiddenVer));
 handles.netfname                       = 'GB-GB-2014-04-10-0000.net';

...
n		= numel(f);
subplot(321)
plot(f,m,'bo','MarkerFaceColor','w','LineWidth',2,'MarkerSize',5);
hold on
% for ii	= 1:n
% 	str = num2str(s(ii));
...
n		= numel(f);
hold on
plot(f,m,'rs','MarkerFaceColor','w','LineWidth',2,'MarkerSize',5);
% for ii	= 1:n
% 	str = num2str(s(ii));
% 	h	= text(f(ii),m(ii),str,'HorizontalAlignment','center','VerticalAlignment','middle');
% 	set(h,'Color','r');
...
n		= numel(f);
hold on
plot(f,m,'kd','MarkerFaceColor','w','LineWidth',2,'MarkerSize',5);
% for ii	= 1:n
% 	str = num2str(s(ii));
% 	h	= text(f(ii),m(ii),str,'HorizontalAlignment','center','VerticalAlignment','middle');
% 	set(h,'Color','k');
...
b		= regstats(m,f,'linear',{'beta','rsquare','tstat','fstat'});
beta3	= b.beta;
str3	= ['Post & Normal: R^2 = ' num2str(b.rsquare,2) ', F_{' num2str(b.fstat.dfe,2) '} = ' num2str(b.fstat.f) ', p = ' num2str(b.fstat.pval,2)];

axis([-10 110 50 110]) 
pa_regline(beta2,'r-');
h = pa_regline(beta1,'b-'); set(h,'LineWidth',2);
h		= pa_text(0.1,0.1,str2); set(h,'Color','r')
h		= pa_text(0.1,0.2,str1);set(h,'Color','b') 
box off;
...
xlabel('Phoneme score (%)');
ylabel('FDG'); 
title({'Temporal Superior Sulcus';'Vision'});
...
xlabel('Phoneme score (%)');
ylabel('FDG'); 
title({'Calcarine sulcus';'Control'});

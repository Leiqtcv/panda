function pet_model_dependentconditions
...
str = ['model {\r\n',...
	...
	'\t\tb0[condIdx] ~ dnorm(mub0[condIdx],taub0[condIdx])\r\n',...
	'\t\ttaub0[condIdx] ~ dgamma(S0,R0)\r\n',...
	'\t\tfor (j in 1:nPredictors) {\r\n',...
	'\t\t\tb[j,condIdx] ~ dnorm(mub[j,condIdx],taub[j,condIdx])\r\n',...
	'\t\t\ttaub[j,condIdx] ~ dgamma(S[j],R[j])\r\n',...
	'\t\t\tmub[j,condIdx] ~ dnorm(0,1.0E-12)\r\n',...
	'\t\t}\r\n',...
	'\tmub0[condIdx] ~ dnorm(0,1.0E-12)\r\n',...
	'\t}\r\n',...
	'#Hyperpriors\r\n',...
	'\tS0 <- pow(meanGamma0,2)/pow(sdGamma0,2)\r\n',...
	'\tR0 <- meanGamma0/pow(sdGamma0,2)\r\n',...
	'\tmeanGamma0 ~ dunif(0.01,30)\r\n',...
	'\tsdGamma0 ~ dunif(0.01,30)\r\n',...
	'\tfor (j in 1:nPredictors) {\r\n',...
	'\tS[j] <- pow(meanGamma[j],2)/pow(sdGamma[j],2)\r\n',...
	'\tR[j] <- meanGamma[j]/pow(sdGamma[j],2)\r\n',...
	'\t\tmeanGamma[j] ~ dunif(0.01,30)\r\n',...
	'\t\tsdGamma[j] ~ dunif(0.01,30)\r\n',...
	'\t}\r\n',...
	'#Missing values\r\n',...
	'\tfor(i in 1:nData) {\r\n',...
	'\t\tfor (j in 1:nPredictors) {\r\n',...
...
for areaIndx = 1:nArea
	...
	for condIdx = 1:nCond
		...
		%% Crossmodal?
		figure(10+condIdx*100)
		subplot(1,3,areaIndx)
		plotPost(bSample(:,2)-bSample(:,1),'compVal',0);
		hold on
		xlim([-2 2]);
		ylim([-2 2]);
		box off
...
r = randn(size(DAT(area).Dprevideo));
nmat = NaN(size(DAT(area).Dprevideo));

pre		= [DAT(area).Dprerest DAT(area).Dprevideo NaN(size(DAT(area).Dprevideo))];
...
for condIdx = 1:nCond
	...
	if condIdx==1
		tmp = zx(sel,:);
		tmp(:,3) = nanmean(tmp(:,2));
		b					= glmfit(tmp,[zy(sel) N(sel)],'binomial','link','logit');
	else

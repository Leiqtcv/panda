...
if ~loadflag
	if true
		...
		str = ['model {\r\n',...
			'\tfor ( i in 1:Ntotal ) {\r\n',...
			'\t\ty[i] ~ dnorm( mu[i],tau)\r\n',...
			'\t\tmu[i] <- a0 + a1[x1[i]] + a2[x2[i]] + a1a2[x1[i],x2[i]] + aS[S[i]]  +(bMetI[x1[i],x2[i]])*xMet[i]\r\n',...
			'\t#Missing values\r\n',...
			'\t\t\tx1[i] ~ dnorm(mux,taux)\r\n',...
			'\t\t\tx2[i] ~ dnorm(mux,taux)\r\n',...
			'\t}\r\n',...
	...
	for roiIdx = 1:nroi
		...
		bMetI		= reshape(samples.bMetI,nChains*chainLength,Nx1Lvl,Nx2Lvl);
		...
		bMetIprior		= reshape(samples.bMetIprior,nChains*chainLength,Nx1Lvl,Nx2Lvl);
...
if b1b2flag
	...
	IMG = [mu(1).img mu(2).img mu(3).img mu(4).img mu(5).img mu(6).img mu(7).img mu(8).img mu(9).img];
	figure(704)
	colormap(col)
	imagesc(IMG')
	hold on
	contour(repmat(braincontour',9,1),1,'k');
	axis equal
	caxis([-3 3])
	colorbar
	set(gca,'YDir','normal');

...
% if true
	...
	% 		'\tlambda[j] ~ dchisqr(1)\r\n',...
	% 		'\t#lambda[j] <- 0.1\r\n',...
	% 		'\t\t#bprior[j] ~ dt(0,3,3) # not for updating\r\n',...
...
% 		];
	...
	for roiIdx = 1:nroi
		area = roiIdx;
		data(area)
		x		= [data(area).FDGrest data(area).FDGvideo];
		y		= data(area).Phoneme;
		
		xprior	= x(12:end,:);
		sel		= isnan(xprior(:,1)) | isnan(xprior(:,2)) ;
		xprior	= xprior(~sel,:);
		...
		for bIdx = 1:nPredictors
			...
			[f2,x2]		= ksdensity(tmp,'kernel','normal','support',[binse(1) binse(end)]); % prior
			
			[~, indk2] = min(abs(x2-crit));
			tmp = bSample(:,bIdx);
			sel = tmp>binse(1)&tmp<binse(end);
			tmp = tmp(sel);
			if sum(sel)
	...
	img2	= roicolourise(p,fname,roifiles,bf(:,2),img);
	IMG2	= [];
	for zIndx = 1:nindx
		tmp		= squeeze(img2(:,:,indx(zIndx)));
		IMG2	= [IMG2;tmp];
	end
	img3	= roicolourise(p,fname,roifiles,bf(:,3),img);
	IMG3	= [];
	for zIndx = 1:nindx
		tmp		= squeeze(img3(:,:,indx(zIndx)));
		IMG3	= [IMG3;tmp];
	end
	...
	muimg2	= roicolourise2(p,fname,roifiles,MU(:,2),img);
	muIMG2	= [];
	for zIndx = 1:nindx
		tmp		= squeeze(muimg2(:,:,indx(zIndx)));
		muIMG2	= [muIMG2;tmp];
	end
...
% 	keyboard
	...
	xlswrite('table3',vroi',1,'A');
	xlswrite('table3',vmap,1,'B');
	xlswrite('table3',rroi',2,'A');
	xlswrite('table3',rmap,2,'B');

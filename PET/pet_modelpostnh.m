...
if ~loadflag
	...
	for roiIdx = 1:nroi
		...
		sel		= ismember(x1,2:3) & ismember(x2,1:3);
		x1		= x1(sel); % 'Post-implant','Pre-implant'
		x1		= x1-1;
		...
		%% RUN THE CHAINS
		parameters		= {'b0','b1','b2','b1b2','b0prior','b1prior','b2prior','b1b2prior','sigma','lambda0','lambda1','lambda2'};	% The parameter(s) to be monitored.
		...
		% q1: Main factor: difference between groups POST & NH
		H1	= b1(:,2)-b1(:,1);
		H0	= b1prior(:,2)-b1prior(:,1);
		bf	= pa_bayesfactor(H1,H0);
		BF(roiIdx).q1 = bf;
		
		% q2: Main factor: difference between stimuli REST & VIDEO
		H1	= b2(:,2)-b2(:,1);
		H0	= b2prior(:,2)-b2prior(:,1);
		bf	= pa_bayesfactor(H1,H0);
		BF(roiIdx).q2 = bf;
		
		% q3: Main factor: difference between stimuli VIDEO & AV
		H1	= b2(:,3)-b2(:,2);
		H0	= b2prior(:,3)-b2prior(:,2);
		bf	= pa_bayesfactor(H1,H0);
		BF(roiIdx).q3 = bf;
		
		% q4: Main factor: difference between stimuli AV & REST
		H1	= b2(:,3)-b2(:,1);
		H0	= b2prior(:,3)-b2prior(:,1);
		bf	= pa_bayesfactor(H1,H0);
		BF(roiIdx).q4 = bf;
		
		% q5-7: Interaction effects
		%		-	-	 - >
		%		Pre Post
		% | R	1,1 1,2
		% | V   2,1 2,2
		...
		b	= squeeze(b1b2(:,1,3));
		H1	= a-b;
		a	= squeeze(b1b2prior(:,2,3));
		b	= squeeze(b1b2prior(:,1,3));
		H0	= a-b;
		bf	= pa_bayesfactor(H1,H0);
		BF(roiIdx).q7 = bf;
	end
	...
	pa_datadir
	save postnhanalysis BF param samp
...
if sum(selbf)
	...
	xlswrite('tablepostnh',a,1,'A2');
	xlswrite('tablepostnh',s,1,'B2');
	p	= NaN(nroi,2);
	for ii = 1:nroi
	...
	p	= p(:,2)-p(:,1);
	xlswrite('tablepostnh',round(p(selbf)*10)/10,1,'C2');
	xlswrite('tablepostnh',round(1./[BF(selbf).q1]'),1,'D2');
end
xlswrite('tablepostnh',{'Brain region'},1,'A1');
xlswrite('tablepostnh',{'Side'},1,'B1');
xlswrite('tablepostnh',{'Beta'},1,'C1');
xlswrite('tablepostnh',{'BF_{10}'},1,'D1');


%% Example
selroi = strcmp([data.roi],'MNI_Temporal_Sup_R_roi.mat');
stmIdx	= 1;
stp		= 25;
y		= data(selroi).FDG(:);
x1		= data(selroi).group(:); % group
x2		= data(selroi).stim(:); % stimulus
sel		= ismember(x1,1:3) & ismember(x2,1:3) & y>30;
x1		= x1(sel)-1;
y		= y(sel);
figure(101)
subplot(131)
...
plot(1:2,bias+b1+b2+b1b2,'Color','k','LineWIdth',2);
sel = x1==1;
sum(sel)
...
box off;
axis square;
set(gca,'TickDir','out','XTick',[1 2 3]-1,'XTickLabel',{'Pre-CI','Post-CI','NH'});
...
pa_text(0.5,0.9,str);
str = ['\beta_{nh} - \beta_{post} = ' num2str(b1(2)-b1(1),2)];
pa_text(0.5,0.8,str);
pa_datadir;
print('-depsc','-painter',[mfilename '_example']);
...
if sum(selbf)
	...
	xlswrite('tablepostnh',a,1,'E2');
	xlswrite('tablepostnh',s,1,'F2');
	p	= NaN(nroi,2);
	for ii = 1:nroi
	...
	p	= p(:,2)-p(:,1);
	xlswrite('tablepostnh',round(p(selbf)*10)/10,1,'G2');
	xlswrite('tablepostnh',round(1./[BF(selbf).q1]'),1,'H2');
end
xlswrite('tablepostnh',{'Brain region'},1,'E1');
xlswrite('tablepostnh',{'Side'},1,'F1');
xlswrite('tablepostnh',{'Beta'},1,'G1');
xlswrite('tablepostnh',{'BF_{10}'},1,'H1');
...
if sum(selbf)
	...
	xlswrite('tablepostnh',a,1,'I2');
	xlswrite('tablepostnh',s,1,'J2');
	p	= NaN(nroi,2);
	for ii = 1:nroi
	...
	p	= p(:,2)-p(:,1);
	xlswrite('tablepostnh',round(p(selbf)*10)/10,1,'K2');
	xlswrite('tablepostnh',round(1./[BF(selbf).q3]'),1,'L2');
end
xlswrite('tablepostnh',{'Brain region'},1,'I1');
xlswrite('tablepostnh',{'Side'},1,'J1');
xlswrite('tablepostnh',{'Beta'},1,'K1');
xlswrite('tablepostnh',{'BF_{10}'},1,'L1');
...
if sum(selbf)
	...
	xlswrite('tablepostnh',a,1,'M2');
	xlswrite('tablepostnh',s,1,'N2');
	p	= NaN(nroi,2);
	for ii = 1:nroi
		p(ii,1)		= param(ii).b2(1);
		p(ii,2)		= param(ii).b2(3);
	...
	p	= p(:,2)-p(:,1);
	xlswrite('tablepostnh',round(p(selbf)*10)/10,1,'O2');
	xlswrite('tablepostnh',round(1./[BF(selbf).q4]'),1,'P2');
end
xlswrite('tablepostnh',{'Brain region'},1,'M1');
xlswrite('tablepostnh',{'Side'},1,'N1');
xlswrite('tablepostnh',{'Beta'},1,'O1');
xlswrite('tablepostnh',{'BF_{10}'},1,'P1');
...
xlswrite('tablepostnh',1./[BF.q5]',1,'K2');
...
xlswrite('tablepostnh',1./[BF.q6]',1,'M2');
p = NaN(nroi,1);
for roiIdx = 1:nroi
	param(roiIdx).b1b2
	a = param(roiIdx).b1b2(2,2);
...
xlswrite('tablepostnh',1./[BF.q7]',1,'O2');
p = NaN(nroi,1);
for roiIdx = 1:nroi
	param(roiIdx).b1b2
	a = param(roiIdx).b1b2(2,3);

function pa_pet_analysis_example_2(subject,cond)
...
if nargin<1 % the Number of INput ARGuments is 0
	subject = 11; % so I will provide this default
end
...
modal		= {'c';'v'}; % Stimulus modality: v = visual, c  = control, av = audiovisual, order is important so usually:
% {'c';'v'} OR {'v';'av'} OR {'c';'av'}, the first is the baseline, the
% second is additional activity
threshdiff  = 20;
...
% 	origin	= [65 65 30]; % S 13 14
	...
	fname	= [sstr num2str(subject) '-' modal{2} '-' cond '.img'];
	file	= [dname '\' fname];
...
%% Comparison
if comp_flag
	files = {[dname '\wdro_' sstr num2str(subject) '-' modal{1} '-' cond];...
		[dname '\wdro_' sstr num2str(subject) '-' modal{2} '-' cond]};

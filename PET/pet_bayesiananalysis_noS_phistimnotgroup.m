function pet_bayesiananalysis_noS_phistimnotgroup
% ANOVATWOWAYJAGSSTZ
%
...
if b1b2flag
	...
	IMG = [mu(1).img mu(2).img mu(4).img mu(5).img mu(6).img mu(7).img mu(8).img mu(9).img];
	figure(704)
	colormap(col)
	imagesc(IMG')
	hold on
	contour(repmat(braincontour',8,1),1,'k');
	axis equal
	caxis([-3 3])
	colorbar
	set(gca,'YDir','normal');

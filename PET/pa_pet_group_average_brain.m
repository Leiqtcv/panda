function pa_pet_group_average_brain(subject,cond)
...
%% Change these
d = 'E:\DATA\KNO\PET\SPM\Normalhearing'; % directory with all swdro-files
cd(d);
fname = 'E:\DATA\KNO\PET\Default\PET.nii'; % the default MNI PET scan
% group = 'normal';
group = 'pre';
% group = 'post';
cond_act = 'v'; % or v
...
for ii = 1:nfiles
	...
	try
	fnameV		= ['swdro_s' n num2str(files(ii)) '-' cond_act '-' group];
	niiV		= load_nii(fnameV);
	view_nii(niiV);
	
	fnameC		= ['swdro_s' n num2str(files(ii)) '-' cond_base '-' group];
	niiC		= load_nii(fnameC);
	view_nii(niiC);
	
	Verschil	= pa_pet_diffscan(niiC,niiV,threshd,threshdiff);
% 	C = niiC.img;
% 	T = niiV.img;
% 	Verschil =(T-C)./C;
% 	Verschil = zscore(Verschil);
	GroupAvg	= GroupAvg + Verschil;
	
	pa_pet_plot_difference(Verschil,niiPET,niiC)
	end
	
end
GroupAvg = GroupAvg/nfiles;
pa_pet_plot_difference(GroupAvg,niiPET,niiC)
...
if nargin<1 % the Number of INput ARGuments is 0
	subject = 16; % so I will provide this default
end
...
%%
%% Difference
function Diff = pa_pet_diffscan(niiScan1,niiScan2,threshd,threshdiff)
C = niiScan1.img;
T = niiScan2.img;
...
function pa_pet_plot_difference(D,niiPET,niiC)
% D(D<0) = 0;
% D(D>3) = 3;

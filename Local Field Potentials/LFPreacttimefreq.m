function [muM,T] = LFPreacttimefreq(data,cfg)
% LFPTIMEFREQ(DATA)
%
...
T = [];
for i = 1:length(data)
    [m,f,t] = gettimefreq(data(i).lfpfilt,1000,data,i);
    M(:,:,i) = m;
    %     if i ==1
    %         M = m;
    %         F = f;
...
function [MTX,Freq,t] = gettimefreq(x,Fs,data,TrialNr)
...
if ReactFlag
    MTX = NaN(size(M));
    RT      = [data(TrialNr).reactiontime];
    onset =  data(TrialNr).stimulus.values(3);
    rt      = RT+300+onset;
    [~,indx]  = min(abs(rt-t));
    n = length(t);
    indx    = (indx-50):n;
    MTX(:,1:length(indx)) = M(:,indx);
end

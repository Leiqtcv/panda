...
v			= [0.5 1 2 4]; % drift rate (units/sec)
nv			= numel(v);
a			= [.2 .1 .05 .025];  % decision level (units)
na			= numel(a);
s			= [0.05 .1 .2 .4];  % standard deviation of drift (units/sec)
ns			= numel(s);
te			= fliplr([0.05 .1 .2 .4]); % indecision time (sec)
nt			= numel(te);
...
col			= jet(nv);
for ii = 1:nv
	Par(3)		= v(ii);  % indecision time (sec)

	[Y,XI]		= fun(Par);
	subplot(421)
	plot(XI,Y,'k-','linewidth',2,'Color',col(ii,:));
	
	subplot(422)
	plot(XI,gradient(Y),'k-','linewidth',2,'Color',col(ii,:));
end
...
col			= jet(na);
for ii = 1:na
	Par(1)		= a(ii);  % indecision time (sec)
...
col			= jet(ns);
for ii = 1:ns
	Par(2)		= s(ii);  % indecision time (sec)
...
col			= jet(nt);
for ii = 1:nt
	Par(4)		= te(ii);  % indecision time (sec)

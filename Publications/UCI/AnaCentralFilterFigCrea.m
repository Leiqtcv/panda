...
BoxplotVsEnInf	=	0;	 %Plots a boxplot as function of both the energetic (PB0) conditions and informational (PB~=0)
ThrVsDist		=	0;	 %Plots the threshold vs. the average distance [in Hz] of masker frequency from signal for all condition
...
CheckMaskerFreq	=	1;   %Plot the Masker Frequencies, only for fixed conditions!			
...
% -- Completed Blocks -- %
pname		=	[	...
					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Rnd\UCImd\'}; ...
					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Fix\UCImd\'}; ...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Rnd\UCI060\'}; ...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Fix\UCI060\'}; ...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Rnd\UCI059\'}; ...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Fix\UCI059\'}; ...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Rnd\UCI058\'}; ...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Fix\UCI058\'}; ...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Rnd\UCI057\'}; ...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Fix\UCI057\'}; ...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Rnd\UCI037\'}; ...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Fix\UCI037\'}; ...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Rnd\UCI033\'};...
% 					{'C:\Users\mdirkx\Documents\Auditory Scene Analysis\CentralFilter\Data\Completed Blocks\Fix\UCI033\'};...
			...
			for x = 1:Nuplow
				...
				for n = 1:Nsubj
					...
					for o	= 1:Nblock   %Nblock
						...
						sel		=	D(:,8) == Utype(k) & D(:,7) == Utime(l) & D(:,17) == Upara(m) & D(:,16) == Usubj(n) & D(:,6) == Ublock(o) & Ublock(x);
						d		=	D(sel,[18 1 19]);
...
xx		=	[-100 max(Df(:,1))*1.1];                 % setting x-axis
yy		=	[min(D(:,1))*1.1 max(D(:,1))*1.1];  % setting y-axis
...
ERB		=	21.4 * log10(4.37 * x*10^-3 + 1 );  %ERB for frequencies x (100:20000)
xfit1	=	Ufreq - x;                          %For x frequencies
xfit2	=	sERB - ERB;							%For all ERB's for the frequencies x, corrected for signal ERB
		...
		for l=1:Ntime
			...
			for m = 1:Npara %for every fix/rnd
				...
				for o	= 1:Nblock
					...
					subplot(r,c,k)
					try
					h1(cnt)	= plot(DF(:,1),Thr,mk,'Color',COL,'MarkerFaceColor',COL,'MarkerEdgeColor',COL); %mark thresholds for every block
					hold on
					catch
						disp('Error');
					end
				...
				for o = 1:Nsubj
					...
					for p = 1:Nblock
						...
						try
							...
							disp('Error Plotting d: Check if Nblocks of individual subjects are mismatch')
						end
					end
				end
					...
					for m = 1:Ntime
						...
						for n = 1:Npb
							...
							sel = D(:,16) == Usubj(i) & D(:,6) == Ublock(j) & D(:,17) == Upara(k) & D(:,8) == Utype(l) & D(:,7) == Utime(m) &...
								D(:,19) == dround(Upb(n)) & D(:,21) == Uuplow(h);
							d	= D(sel,22:37);
							Thr		= D(sel,1);
...
% 							pa_horline(0.5,'k.-', Avgdist, Ufreq);	%plot horizontal line from SigFreq +/- AvgDist
							Xtick		   =	[100 200 800 1600 3200 6400 12800];
							Xticklabel	   =	Xtick;
							set(gca,'Xscale','Log','XTick',Xtick,'XTickLabel',Xticklabel,'Fontsize',16,'Ytick',[])
							title(['Protected Band 2 Oct'],'fontweight','b','fontsize',16);
							xlabel('Frequency [Hz]','fontsize',16)%200 Hz + n*(1/6) Octaves
							axis([0 Flist(end) 0  1]) %max(sum(NN,2))]);
							%ylabel('Times frequency (masker/signal) occurred during one trial')
							keyboard
							
							patch([(0.0224*10^4) PBFreq(1) PBFreq(1) (0.0224*10^4)],[0 0 1 1],[1 0.5 0.5])
							patch([ PBFreq(2) (1.1404*10^4) (1.1404*10^4) PBFreq(2)],[0 0 1 1],[1 0.5 0.5])
						end
					end
				end
			end
...
%% Helpers
function [D,T] = loaddat(pname,CheckMaskerFreq,Check)
...
for k=1:len
	...
	for l=1:Ndat
		...
		if CheckMaskerFreq == 1
		D(cnt,:)		=	[Thr Sfreq(1) Sloc Mloc Mspl NBlock Timing Type Nmask BaseRate ProtectBand CB1_3 Sspl Subject Folder...
			Mfreq dround(Cn.ProtectBand) AvgDiff UpLow MaskFreqHz];	%#ok<AGROW>
		elseif CheckMaskerFreq == 0
			D(cnt,:)		=	[Thr Sfreq(1) Sloc Mloc Mspl NBlock Timing Type Nmask BaseRate ProtectBand CB1_3 Sspl Subject Folder...
			Mfreq dround(Cn.ProtectBand) AvgDiff UpLow];
		end
		cnt				=	cnt + 1;
	end
end

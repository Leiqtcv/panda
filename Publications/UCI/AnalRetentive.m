...
if MovieFlag
	...
	for jj = 1:n
		...
		cla;
		hold on;
		md = w(jj).*mdFix1 + (1-w(jj)).*mdRnd1;
		iqr = w(jj).*iqrFix1 + (1-w(jj)).*iqrRnd1;
		semilogy(md,iqr,'ko','MarkerFaceColor',col(1,:));
		
		md = w(jj).*mdFix2 + (1-w(jj)).*mdRnd2;
		iqr = w(jj).*iqrFix2 + (1-w(jj)).*iqrRnd2;
		semilogy(md,iqr,'ko','MarkerFaceColor',col(2,:));
		
		md = w(jj).*mdFix3 + (1-w(jj)).*mdRnd3;
		iqr = w(jj).*iqrFix3 + (1-w(jj)).*iqrRnd3;
		semilogy(md,iqr,'ko','MarkerFaceColor',col(3,:));
		
		md = w(jj).*mdFix4 + (1-w(jj)).*mdRnd4;
		iqr = w(jj).*iqrFix4 + (1-w(jj)).*iqrRnd4;
		semilogy(md,iqr,'ko','MarkerFaceColor',col(4,:));
		
		x = 36:60;
		y = b2.beta(2)*x+b2.beta(1);
		y = 10.^y;
		semilogy(x,y,'k-','LineWidth',2);
		
		x = 0:37;
		y = b1.beta(2)*x+b1.beta(1);
		y = 10.^y;
		semilogy(x,y,'k-','LineWidth',2);
		axis square;
		axis([0 70 4 60]);
		
		set(gca,'Yscale','log','YTick',log10(logspace(4,60,10)),'YTickLabel',round(log10(logspace(4,60,10))));
		xlabel('Median threshold (dB)');
		ylabel('IQR threshold (dB)');
...
for kk = 1:nFreq
	for jj = (1:nOnset)-1
		k = k+1;
		seltype = Onset==jj & Freq==kk & Memory==0; % TcFc, no memory
		T = NaN(nPB,3);
		for ii = 1:nPB
			selpb = PB==uPB(ii);
			sel = seltype & selpb;
		...
		h(k) = errorbar(uPB+k*0.025-2.5*0.025,T(:,2),T(:,2)-T(:,1),T(:,3)-T(:,2),'ko-','LineWidth',2,'MarkerFaceColor','w','Color',col(k,:));
		hold on
		Drempel(k).T = T;
...
for kk = 1:nFreq
	for jj = (1:nOnset)-1
		...
		h(k) = errorbar(uPB+k*0.025-2.5*0.025,T(:,2),T(:,2)-T(:,1),T(:,3)-T(:,2),'ks-','LineWidth',2,'MarkerFaceColor','w','Color',col(k,:));
		hold on
		Drempel(k).T = T;
...
for ii = 1:4
	...
	iqr = t(:,3)-t(:,1);
	% 	iqr = t(:,1);
	semilogy(md,iqr,'ks-','MarkerFaceColor',col(ii,:));
	hold on
...
semilogy(mu,iqr,'k-','LineWidth',2);
% pa_verline([ceil floor]);
% ylim([1 100])
% yt = pa_oct2bw(1,0:5);
% set(gca,'YTick',yt,'YTickLabel',yt);
% axis square

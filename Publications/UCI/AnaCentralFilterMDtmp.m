function AnaCentralFilterMDtmp
...
ERBScale	=	1;   %Plots everything on ERB scale
ThrVsBlock  =   0;   %Plots thresholds for every PB
...
sERB	=	21.4 * log10(4.37 * Ufreq*10^-3 + 1 ); %signal ERB
mERB	=	21.4 * log10(4.37 * MF(:,1)*10^-3 + 1 ); %masker ERB
F		=	sERB - mERB;

Df		=	Ufreq-MF(:,1);                      %corrected protected bands??

xx		=	[-100 max(Df)*1.1];                 % setting x-axis
yy		=	[min(D(:,1))*1.1 max(D(:,1))*1.1];  % setting y-axis

% Fl		=	Ufreq / 2^0.5;
% Fu		=	Ufreq * 2^0.5;
% CB		=	Fu - Fl;
CB		=	(4.37 * Ufreq*10^-3 + 1) * 24.7;    %Characteristic frequency
x		=	100:20000;                          %??
y		=	exp( -( (x-Ufreq) ./ CB ).^2 );     %??
yfit	=	fitroex(x,y,Ufreq);                 %??

ERB		=	21.4 * log10(4.37 * x*10^-3 + 1 );  %ERB
xfit1	=	Ufreq - x;                          %
xfit2	=	sERB - ERB;
...
for k=1:Ntype               %length type (iR vs iC)
	...
	for l=1:Ntime                   %for every (a)synchronous condition
		...
		for m = 1:Npara %for every fix/rnd
			...
			for n	= 1:Nblock        %Setting points
				
				sel		=	D(:,8) == Utype(k) & D(:,7) == Utime(l) & D(:,17) == Upara(m) & D(:,6) == Ublock(n);   %for every unique iC/iR; sync/async; fix/rnd; Block
				d		=	D(sel,[18 1 19]);   % create matrix of Threshold/SDM/PBoct for above selected rows
				[~,idx]	=	sort(d(:,3));       % sort by PBoct
				d		=	d(idx,:);           % apply sort to Thr and SDM
				md(:,n)	=	d(:,2);             % apply thresholds in md (for different blocks)
				
				Maxd	=	max(d(:,2));
				att		=	d(:,2)-Maxd;
				% 	 		F		=	d(:,3);
				matt(:,n)	=	att;            % same but then for matt
				
				subplot(r,c,k)
				h1(cnt)	= plot(Df,mean(md,2),mk,'Color',col,'MarkerFaceColor',col,'MarkerEdgeColor',col); %mark thresholds for every block
				hold on
				
				subplot(r,c,k+2)
				h2(cnt)	= plot(F,mean(matt,2),mk,'Color',col,'MarkerFaceColor',col,'MarkerEdgeColor',col);
				hold on
...
xx		=	[0.8 max(Nblock)*1.1];  %setting x-axis
	...
	for m=1:Ntype               %length type (iR vs iC)
		...
		for l=1:Ntime                   %for every (a)synchronous condition
			...
			if Utime(l) == 0 %asynchronous
				...
				xlabel('Block number')
				ylabel(Tstr)
				title([Str ' ' Pstr ' f_{sig}= ' num2str(Ufreq) ' Hz'])
				axis('square')
				legend(h1,L,'Location','NorthEast')
				legend('boxoff')
			elseif Utime(l) == 1 %synchronous

...
for kk = 1:nFreq
	for jj = (1:nOnset)-1
		...
		h(k) = errorbar(uPB+k*0.025-2.5*0.025,T(:,2),T(:,2)-T(:,1),T(:,3)-T(:,2),'ko-','LineWidth',2,'MarkerFaceColor',col(k,:));
		hold on
		Drempel(k).T = T;
...
for kk = 1:nFreq
	for jj = (1:nOnset)-1
		...
		errorbar(uPB+k*0.05-2.5*0.05,T(:,2),T(:,2)-T(:,1),T(:,3)-T(:,2),'ks-','LineWidth',2,'Color',col(k,:));
		hold on
		h(k) = plot(uPB+k*0.05-2.5*0.05,T(:,2),'ks','LineWidth',2,'MarkerFaceColor',col(k,:));
		Drempel(k).T = T;
...
% Y = limitfun(X,beta0);
%
% semilogy(X,Y,'ko','MarkerFaceColor','w');
% hold on
%
% sd		= 10;
% ceil	= 50;
...
% % beta	= nlinfit(X,Y,@limitfun,beta0);
% x = 0:100;
% y = limitfun(x,beta);
% semilogy(x,y,'k-','Color','r','LineWidth',2);

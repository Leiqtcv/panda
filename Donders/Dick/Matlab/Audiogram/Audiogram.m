...
eval('path(path,''C:\dick\Audiogram\Matlab'')');
...
function Audiogram_init(hObject)
    ...
    data.cum = 0;
    data.cfg_freq = [1000 500 250 125 1000 2000 3000 4000 6000 8000 16000];
    ...
    evalin('base',addPath);
    circuitRP2 = strcat(get(data.edit_mapRCO,'String'),'\Rp2.rco');

    disp('Initialize TDT .........');
%---Active X
    [zBus   err(1)] = ZBUS(4); % number of racks
...
function Audiogram_run(hObject)
    ...
    guidata(hObject, data);
%------------------------------------------------------------------%
function push_init_Callback(hObject, eventdata, handles)

...
void CAcousticType::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_RADIO4, r_NoSound);
	DDX_Control(pDX, IDC_RADIO5, r_StatDyn);
...
BEGIN_MESSAGE_MAP(CAcousticType, CDialog)
	...
	ON_BN_CLICKED(IDC_Apply, &CAcousticType::OnBnClickedApply)
...
// apply
void CAcousticType::OnBnClickedApply()
{
	CMonkeyApp *pnt = (CMonkeyApp *) AfxGetApp()->m_pMainWnd;
	
	pnt->MarkMenuItem(ID_ACOUSTICSTIMULI_TONE,r_Tone.GetCheck());
	pnt->MarkMenuItem(ID_ACOUSTICSTIMULI_NOISE,r_Noise.GetCheck());
	pnt->MarkMenuItem(ID_ACOUSTICSTIMULI_RIPPLE,r_Ripple.GetCheck());
	pnt->MarkMenuItem(ID_ACOUSTICSTIMULI_NOSOUND,r_NoSound.GetCheck());
	saveData();
	setApplyGray();
}
// close

...
int skyData[2][12]; // intensity red/green per spoke,
					// each spoke 6 rings
// sound
CString boardSelect  = "4E";
CString muteSelect   = "72";
CString sourceSelect = "40";
...
void execError(void)
{
	...
	int tmp[] = {5,6,8,9}; 	// barL,barUp,rew2
	reactionTime = -(preDim - (newTime - realStart));
	pressTime = newTime - realStart;
	for (n = 0; n < 4; n++)	// disable bar and reward2
	{
		i = tmp[n];
	...
	while (GoOn)
	{
		...
		if (stateTrial == stateRunTrial)
		{
			...
			for (int i=0; i<nStim; i++)
			{
				...
				if (m != 0) state |= (1 << i);
				n += sprintf(outBuf+n,"%2d",m);
			}
			FSM_Status.Format("%s",outBuf);
			trialStatus = state;
			if (!finishStimulus)
			{

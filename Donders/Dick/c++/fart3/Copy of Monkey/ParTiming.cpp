...
void CParTiming::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT2, m_TargetFixed);
	DDX_Text(pDX, IDC_EDIT3, m_TargetRandom);
	DDX_Text(pDX, IDC_EDIT4, m_TargetChanged);
	DDX_Text(pDX, IDC_EDIT5, m_RandomTarget);
	DDX_Text(pDX, IDC_EDIT6, m_ReactFrom);
	DDX_Text(pDX, IDC_EDIT7, m_ReactTo);
	DDX_Control(pDX, IDC_SPIN1, s_Fixation);
	DDX_Control(pDX, IDC_SPIN2, s_TargetFixed);
	DDX_Control(pDX, IDC_SPIN3, s_TargetRandom);
	DDX_Control(pDX, IDC_SPIN4, s_TargetChanged);
	DDX_Control(pDX, IDC_SPIN6, s_ReactFrom);
	DDX_Control(pDX, IDC_SPIN7, s_ReactTo);
}

BEGIN_MESSAGE_MAP(CParTiming, CDialog)
	ON_NOTIFY(UDN_DELTAPOS, IDC_SPIN1, &CParTiming::OnDeltaposSpin1)
	ON_NOTIFY(UDN_DELTAPOS, IDC_SPIN2, &CParTiming::OnDeltaposSpin2)
	ON_NOTIFY(UDN_DELTAPOS, IDC_SPIN3, &CParTiming::OnDeltaposSpin3)
	ON_NOTIFY(UDN_DELTAPOS, IDC_SPIN4, &CParTiming::OnDeltaposSpin4)
	ON_NOTIFY(UDN_DELTAPOS, IDC_SPIN6, &CParTiming::OnDeltaposSpin6)
	ON_NOTIFY(UDN_DELTAPOS, IDC_SPIN7, &CParTiming::OnDeltaposSpin7)
	ON_BN_CLICKED(IDOK, &CParTiming::OnBnClickedOk)
	ON_BN_CLICKED(IDCANCEL, &CParTiming::OnBnClickedCancel)
	ON_BN_CLICKED(IDC_Apply, &CParTiming::OnBnClickedApply)
END_MESSAGE_MAP()
...
// Target Fixed
void CParTiming::OnDeltaposSpin2(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
...
// Target Random
void CParTiming::OnDeltaposSpin3(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
...
void CParTiming::OnDeltaposSpin7(NMHDR *pNMHDR, LRESULT *pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
...
// apply
void CParTiming::OnBnClickedApply()
{
	saveData();
	setApplyGray();
}

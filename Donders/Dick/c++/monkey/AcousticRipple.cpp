...
void CAcousticRipple::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT23, m_Tones);
	DDX_Text(pDX, IDC_EDIT25, m_Octaves);
	...
	DDX_Control(pDX, IDC_SPIN23, s_Tones);
	DDX_Control(pDX, IDC_SPIN25, s_Octaves);
...
void CAcousticRipple::saveData(void)
{
	...
	pnt->getSettings()->Acoustic3.Tones =
					s_Tones.GetPos() & 0xFFFF;
	pnt->getSettings()->Acoustic3.Octaves =
					s_Octaves.GetPos() & 0xFFFF;
...
void CAcousticRipple::loadData(void)
{
	...
	// Tones
	n = pnt->getSettings()->Acoustic3.Tones;
	s_Tones.SetRange(10,20); 
	s_Tones.SetPos(n); m_Tones.Format("%d",n);
	// Octaves
	n = pnt->getSettings()->Acoustic3.Octaves;
	s_Octaves.SetRange(1,6); 
	s_Octaves.SetPos(n); m_Octaves.Format("%d",n);
	// Ripple phase
...
void CAcousticRipple::OnDeltaposSpin23(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	int Low, High, nPos;
	s_Tones.GetRange(Low, High);
	nPos  = pNMUpDown->iPos;
	nPos += pNMUpDown->iDelta;

	if ((nPos <= High) && (nPos >= Low))
	{
		m_Tones.Format("%d",nPos);
		UpdateData(false);
		*pResult = 0;
	} else *pResult = 1;
	setApplyGreen();
...
void CAcousticRipple::OnDeltaposSpin25(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	int Low, High, nPos;
	s_Octaves.GetRange(Low, High);
	nPos  = pNMUpDown->iPos;
	nPos += pNMUpDown->iDelta;

	if ((nPos <= High) && (nPos >= Low))
	{
		m_Octaves.Format("%d",nPos);
		UpdateData(false);
		*pResult = 0;
	} else *pResult = 1;
	setApplyGreen();

	...
	/*								M O N K E Y							 */
	/*-------------------------------------------------------------------*/
	/*	Versie 1.0 DJH	16-december-2011								 */
	/*		Trainen van apen met auditieve en visuele stimuli			 */
	/*		Functionaliteit van het Pascal programma					 */
	/*		"Training on Leds & Sound" van H. Versnel is als vertrekpunt */
	/*		genomen.													 */ 
	/*																 	 */
	/*-------------------------------------------------------------------*/
	/*	Versie 2.0 DJH 30-januari-2012									 */
	/*		Speakers zijn nu te selecteren.								 */
	/*		Nog probleem met ring 5, spaken 8-12.						 */
	/*		Nakijken als opstelling wordt verplaatst.					 */
	/*		Creeren/runnen passief protocol (*.pro).					 */
	/*														 			 */
	/*-------------------------------------------------------------------*/
	/*  To-Do                                                            */
	/*		controleren van de range van de verschillende parameters     */
	/*		nakijken speakers ring 5, spaken 8-12                        */
	/*		nakijken intensiteit (PWM) regeling leds, hardware probleem? */
	/*********************************************************************/
...
CString Version = "Version 2.1, 31-January-2012";
...
static	Protocol_Record1 Protocol_fixed;
static	Protocol_Record2 Protocol_vary;
static	Protocol_Record2 Protocol_save;

static CString			dataFileName;
static FILE				*dataFile;
static FILE				*logFile;
static FILE				*protFile;
...
BEGIN_MESSAGE_MAP(CMonkeyApp, CWinApp)
	...
	ON_COMMAND(ID_PROTOCOL_CREATE, &CMonkeyApp::OnProtocolCreate)
	ON_COMMAND(ID_PROTOCOL_RUN, &CMonkeyApp::OnProtocolRun)
	ON_COMMAND(ID_VALVE_OPEN, &CMonkeyApp::OnValveOpen)
	ON_COMMAND(ID_VALVE_CLOSE, &CMonkeyApp::OnValveClose)
END_MESSAGE_MAP()
...
BOOL CMonkeyApp::InitInstance()
{
	...
	FILE *pFile;
	bool foundCFG = false;
	settings.Files.cfg = "C:\\Monkey\\Monkey.cfg";
	pFile = fopen(settings.Files.cfg,"r");
	foundCFG = (pFile != 0);
	...
	if (foundCFG)
	{
		...
		settings.Acoustic3.Tones	   = v17;
		settings.Acoustic3.Octaves     = v18;
	...
	else
	{
		...
		settings.Acoustic3.Tones       =    20;
		settings.Acoustic3.Octaves     =     6;
	...
	pPassive = new CPassive();
	pPassive->Create(IDD_PASSIVE, NULL);
	pPassive->ShowWindow(SW_HIDE);
	...
	pIO = new CIO;
	pIO->Create(IDD_IO,NULL);
	pIO->ShowWindow(SW_SHOW);
	...
	pIO->SetWindowPos(NULL,xl,yt,cx,cy,SWP_NOSIZE);
	pIO->ShowWindow(SW_SHOW);
	...
	MarkMenuItem(ID_ACOUSTICSTIMULI_TONE,getSettings()->Acoustic0.tone);
	MarkMenuItem(ID_ACOUSTICSTIMULI_NOISE,getSettings()->Acoustic0.noise);
	MarkMenuItem(ID_ACOUSTICSTIMULI_RIPPLE,getSettings()->Acoustic0.ripple);
	MarkMenuItem(ID_ACOUSTICSTIMULI_NOSOUND,getSettings()->Acoustic0.noSound);
...
void CMonkeyApp::redrawAll(void)
{
	...
	if (passiveFlag)
	{
		...
		pPassive->UpdateInfo(numTrials,curTrial);
	}
	else
	{
		pBar->redraw();
		...
		pHisto->redraw();
		pCum->redraw();
		if (settings.Acoustic0.noSound) kind = 1;
		if (settings.Parameters2.NoLed) kind = 2;
		if (kind == 0) kind = 3;	// vis + aud
		...
		if (runFlag)
		{
			if (trialStatus == 0)
			{
				...
				case 1:
					...
					break;
				case 2:
					getSettings()->TrialInfo.auditive[0]++;
					break;
				case 3:
					getSettings()->TrialInfo.visAud[0]++;
					break;
				}
				...
				if (dataRecord[10] > 0)
				{
					...
					case 1:
						...
						break;
					case 2:
						getSettings()->TrialInfo.auditive[1]++;
						break;
					case 3:
						getSettings()->TrialInfo.visAud[1]++;
						break;
					}
		...
		if (newTime != runTime)
		{
			...
			int m = (newTime /   60) % 60;
		    int s = (newTime %   60);
			int s1= getSettings()->TrialInfo.numTrial;
			pInfo->m_Trial.Format("Time: %d %02d:%02d     Trial: #%d",h,m,s,s1);
			CString str;
...
void CMonkeyApp::OnStart()
{
	...
	if (Dlg.DoModal() == IDOK)
	{
		str1 = settings.Files.data;
		// open 'w', overwrite existing file 
		// lateron open 'a+', append and remove EOF
		dataFileName = str1;
		str1.Replace(".dat",".log");
		settings.Files.log = str1;
		if ((dataFile = fopen(dataFileName,"w")) != NULL)
		{
			fprintf(dataFile,"File: %s\n",dataFileName);
			fprintf(dataFile,"# time fix tarF tar chan r s int1 int2 led snd freq MF MD omega atten reactL reactH F T D latency press rew1 rew2\n");
			fclose(dataFile);
		...
		if (ok)
		{
				pIO->m_IO_Data = settings.Files.data;
				pIO->UpdateData(false);
	...
	{
		getSettings()->Files.data  = pIO->m_IO_Name;
	}
}
...
void CMonkeyApp::OnAppExit()
{
	...
	LPRECT pRect = &rect;
	FILE *pFile;
	pFile = fopen(settings.Files.cfg,"w");
	...
	v17=settings.Acoustic3.Tones;
	v18=settings.Acoustic3.Octaves;
	...
	pIO->GetWindowRect(pRect);
	fprintf(pFile,"%3d %d %d %d %d\n",108,rect.left,rect.right,rect.top,rect.bottom);
...
{
	CString str;
	str = settings.Files.log;
	fprintf(pFile,"Monkey : %s\n",Version);
...
void CMonkeyApp::SaveOutputData(CString FileName)
{
	...
	pFile = fopen(FileName,"a+");
	fprintf(pFile,"%3d %s %4d %4d %4d %4d %d %2d %d %d %d %2d %4d %4d %3d %2d %3d %4d %4d %4d %4d %4d %5d %5d %3d %4d\n",
		getSettings()->TrialInfo.numTrial,			// trial number
		cTime,
...
CString CMonkeyApp::skipLine(void)
{
	CString str = "";
	char ch = 0;
	...
	while (ch != char(10))
	{
		fscanf(protFile,"%c",&ch);
		str = str + ch;
	}
	return str;
}
void CMonkeyApp::execProtocol(void)
{
	CString str;
	int tar, dim, modFreq, mod, freq, n;
	if ((protFile = fopen(settings.Files.prot,"r")) != NULL)
	{
		str = skipLine(); // read header 1
		fscanf(protFile,"%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d%d",
			&Protocol_fixed.num,
			&Protocol_fixed.fixation,
			&Protocol_fixed.targetFixed,
			&Protocol_fixed.targetChanged,
			&Protocol_fixed.led,
			&Protocol_fixed.snd,
			&Protocol_fixed.sndType,
			&Protocol_fixed.fixIStar,
			&Protocol_fixed.ledFix,
			&Protocol_fixed.ledTar,
			&Protocol_fixed.intFix,
			&Protocol_fixed.intTar,
			&Protocol_fixed.intChanged,
			&Protocol_fixed.percChanged,
			&Protocol_fixed.statDyn,
			&Protocol_fixed.tones,
			&Protocol_fixed.octaves,
			&Protocol_fixed.phaseF0,
			&Protocol_fixed.freeze);

		str = skipLine();	// read header 2
		str = skipLine();	// skip garbage
		/* change FSM */
		stims[0].status = statDone;
		stims[1].status = statDone;
		stims[5].status = statDone;
		stims[6].status = statDone;
		stims[7].status = statDone;
		stims[9].status = statDone;
		stims[2].start[0] =    0;
		stims[2].start[1] =	   0;
		stims[2].stop[0]  =    0;
		fscanf(protFile,"%d%d%d%d%d%d%d%lf%d",
			&Protocol_vary.index,
			&Protocol_vary.tarRand,
			&Protocol_vary.ring,
			&Protocol_vary.spoke,
			&Protocol_vary.freq,
			&Protocol_vary.modFreq,
			&Protocol_vary.modDepth,
			&Protocol_vary.density,
			&Protocol_vary.atten);
		
		if (Protocol_fixed.sndType == 3)
		{
			RippleRecord.velocity   = Protocol_vary.modFreq;
			RippleRecord.density    = Protocol_vary.density;
			RippleRecord.modulation = Protocol_vary.modDepth;
			RippleRecord.durStat    = Protocol_fixed.targetFixed + Protocol_vary.tarRand;
			RippleRecord.durRipple  = Protocol_fixed.targetChanged;
			RippleRecord.F0         = Protocol_vary.freq;	
			RippleRecord.Tones      = Protocol_fixed.tones;
			RippleRecord.Octaves    = Protocol_fixed.octaves;
			RippleRecord.PhiF0      = Protocol_fixed.phaseF0;
			RippleRecord.rate       = sampleRate;
			RippleRecord.dynStat    = Protocol_fixed.statDyn;
			RippleRecord.freeze     = Protocol_fixed.freeze;
			pRipple->StartRipple();
			while (pRipple->Busy());
		}
		if (Protocol_fixed.sndType == 0) str = "C:\\Monkey\\RPvdsEx\\noSound.rco";
		if (Protocol_fixed.sndType == 1) str = "C:\\Monkey\\RPvdsEx\\tone.rco";
		if (Protocol_fixed.sndType == 2) str = "C:\\Monkey\\RPvdsEx\\noise.rco";
		if (Protocol_fixed.sndType == 3) str = "C:\\Monkey\\RPvdsEx\\ripple.rco";
		pTDT3->LoadSoundRCO(str);
//		tar		= Protocol_fixed.targetFixed + Protocol_vary.tarRand;
//		dim		= Protocol_fixed.targetChanged;
//		modFreq	= Protocol_vary.modFreq;
//		mod		= Protocol_vary.modDepth;
//		freq	= Protocol_vary.freq;
//		pTDT3->resetSndBuffer();
//		pTDT3->SetAtten(20.0);
//		if (Protocol_fixed.sndType == 0) loadSound_noSound();
//		if (Protocol_fixed.sndType == 1) loadSound_tone(tar, dim, modFreq, mod, freq);
//		if (Protocol_fixed.sndType == 2) loadSound_noise(tar, dim, modFreq, mod);
//		if (Protocol_fixed.sndType == 3) loadSound_ripple(tar, dim);
		passiveFlag = true;
		NextPassiveTrial();
		numTrials = Protocol_fixed.num;
		curTrial  = 1;
	}
	else
	{
...
void CMonkeyApp::NextPassiveTrial(void)
{
	/////////////////////////// attenuation ///////////////////////////
#ifdef _HARDWARE
	pTDT3->SetAtten((double) Protocol_vary.atten);
#endif
	int nStim = 9;
	...
	stims[2].start[1] =	   0;
	stims[2].stop[0]  =    0;
	stims[2].stop[1]  = Protocol_fixed.fixation;
	stims[3].stop[1]  = Protocol_fixed.targetFixed+Protocol_vary.tarRand;
	stims[3].pos[0]   = Protocol_vary.ring;
	stims[3].pos[1]   = Protocol_vary.spoke;
	stims[4].stop[1]  = Protocol_fixed.targetChanged;
	stims[4].pos[0]   = Protocol_vary.ring;
	stims[4].pos[1]   = Protocol_vary.spoke;
	/////////////////////////// attenuation ///////////////////////////
#ifdef _HARDWARE
	pTDT3->SetAtten((double) Protocol_vary.atten);
#endif
	///////////////////////////      SND    ///////////////////////////
	pTDT3->resetSndBuffer();
	int tar		= Protocol_fixed.targetFixed + Protocol_vary.tarRand;
	int dim		= Protocol_fixed.targetChanged;
	int modFreq	= Protocol_vary.modFreq;
	int mod		= Protocol_vary.modDepth;
	int freq	= Protocol_vary.freq;
	if (Protocol_fixed.sndType == 0) loadSound_noSound();
	if (Protocol_fixed.sndType == 1) loadSound_tone(tar, dim, modFreq, mod, freq);
	if (Protocol_fixed.sndType == 2) loadSound_noise(tar, dim, modFreq, mod);
	if (Protocol_fixed.sndType == 3) loadSound_ripple(tar, dim);
	...
	dataRecord[3] = Protocol_fixed.fixation;
	dataRecord[4] = Protocol_fixed.targetFixed + Protocol_vary.tarRand;
	dataRecord[5] = Protocol_fixed.targetChanged;
	dataRecord[6] = nStim;
	dataRecord[7] = 1;  // always finish stimulus
	trialStatus = 1; 
	FSMcmd = 1;

	Protocol_save = Protocol_vary;
...
void CMonkeyApp::NextTrial(void)
{
	...
	if (getSettings()->Acoustic0.ripple)
	{
		...
		if (!pRipple->Busy())
		{
			...
			RippleRecord.Tones      = getSettings()->Acoustic3.Tones;
			RippleRecord.Octaves    = getSettings()->Acoustic3.Octaves;
			RippleRecord.PhiF0      = getSettings()->Acoustic3.Phase;
			RippleRecord.rate       = sampleRate;
			RippleRecord.dynStat    = getSettings()->Acoustic0.dynStat;
			pRipple->StartRipple();
		...
		if (getSettings()->Parameters3.Press == 0)
			...
			stims[7].stop[1] = getSettings()->Parameters3.Unit;
		stims[9].start[1] = getSettings()->Parameters3.Latency;
		stims[9].stop[1] = getSettings()->Parameters3.Latency+
			getSettings()->Parameters3.Factor*getSettings()->Parameters3.Unit;
	...
	if (getSettings()->Acoustic0.dynStat)
		...
		getSettings()->TrialInfo.sndType = -1*prevSaveSound.sndType;
	else
		getSettings()->TrialInfo.sndType = prevSaveSound.sndType;
...
void CMonkeyApp::loadSound_tone(int tar, int dim, int modFreq, int mod, int freq)
{
	...
	{
		CString str = "C:\\Monkey\\RPvdsEx\\tone.rco";
		pTDT3->LoadSoundRCO(str);
		LastSoundRCO = "tone";
	}
...
void CMonkeyApp::loadSound_ripple(int tar, int dim)
{
	if (LastSoundRCO != "ripple")
	{
		CString str = "C:\\Monkey\\RPvdsEx\\ripple.rco";
		pTDT3->LoadSoundRCO(str);
		LastSoundRCO = "ripple";
	}
	pTDT3->resetSndBuffer();
...
CString CMonkeyApp::getVersion(void)
{
	return Version;
}

void CMonkeyApp::OnProtocolCreate()
{
	CProtMake Dlg;
	FILE *pFile;
	int n, nTrials, fix, tarF, tarR, dim;
	CString str;

	Dlg.DoModal();
	pIO->m_IO_Prot = settings.Files.prot;
	pIO->UpdateData(false);
	
	int type = 0;
	int ring, spoke;
	int F0=0, MF=0, MD=0, atten=0, Fd;
	double den;
	if (settings.Acoustic0.tone   == 1) type = 1;
	if (settings.Acoustic0.noise  == 1) type = 2;
	if (settings.Acoustic0.ripple == 1) type = 3;
	
	dataFileName = settings.Files.prot;
	if ((pFile = fopen(dataFileName,"w")) != NULL)
	{
		nTrials = settings.Files.nTrials;
		fprintf(pFile,"# fix tarf dim led snd st ft lf lt if it id pc sd nt no ph fr\n");
		fprintf(pFile,"%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
				nTrials,							// number of trials
				settings.Parameters1.Fixation,		// fixation
				settings.Parameters1.TargetFixed,	// target fixed
				settings.Parameters1.TargetChanged,	// changed target
				settings.Parameters2.NoLed,			// visual ?
				settings.Acoustic0.noSound,			// auditory ?
				type,								// none, tone, noise, ripple
				settings.Parameters2.FixTar,		// fixation = target ?
				settings.Parameters2.FixRed,		// fixation led red ?
				settings.Parameters2.TarRed,		// target led red ?
				settings.Parameters2.Fixation,		// intensity fixation led
				settings.Parameters2.Target,		// intensity target led
				settings.Parameters2.TargetChanged,	// intensity target changed led
				settings.Parameters2.PerChanged,	// penentage changed intensity
				settings.Acoustic0.dynStat,			// dynamic + static ?
				settings.Acoustic3.Tones,			// tones per octave
				settings.Acoustic3.Octaves,			// number of octaves
				settings.Acoustic3.Phase,			// phase F0
				settings.Acoustic3.Freeze			// keep components phases
				);
		fprintf(pFile,"N tarR r s frq mf md den att\n");
		for (n=0; n<nTrials; n++)
		{
			tarR = 	GetRandom(0, settings.Parameters1.TargetRandom);
			ring = GetRandom(settings.Parameters2.Minimum,settings.Parameters2.Maximum);
			if (ring == 0)
				spoke = 1;
			else
				spoke = GetRandom(1,12);
			if (type == 1) // tone
			{
				F0 = settings.Acoustic1.CarrierF;
				Fd = settings.Acoustic1.CarrierFd;
				if (settings.Acoustic1.CarrierFv == 1) 
					F0 += GetRandom(-Fd, Fd);
				MF = settings.Acoustic1.ModF;
				Fd = settings.Acoustic1.ModFd;
				if (settings.Acoustic1.ModFv == 1) 
					MF += GetRandom(-Fd, Fd);
				MD = settings.Acoustic1.ModD;
				Fd = settings.Acoustic1.ModDd;
				if (settings.Acoustic1.ModDv == 1) 
					MD += GetRandom(-Fd, Fd);
				atten = settings.Acoustic1.Atten;
				Fd = settings.Acoustic1.Attend;
				if (settings.Acoustic1.Attenv == 1) 
					atten += GetRandom(-Fd, Fd);
			}
			if (type == 2) // noise
			{
				MF = settings.Acoustic2.ModF;
				Fd = settings.Acoustic2.ModFd;
				if (settings.Acoustic2.ModFv == 1) 
					MF += GetRandom(-Fd, Fd);
				MD = settings.Acoustic2.ModD;
				Fd = settings.Acoustic2.ModDd;
				if (settings.Acoustic2.ModDv == 1) 
					MD += GetRandom(-Fd, Fd);
				atten = settings.Acoustic2.Atten;
				Fd = settings.Acoustic2.Attend;
				if (settings.Acoustic2.Attenv == 1) 
					atten += GetRandom(-Fd, Fd);
			}
			if (type == 3) // ripple
			{
				F0 = settings.Acoustic3.CarrierF;
				Fd = settings.Acoustic3.CarrierFd;
				if (settings.Acoustic3.CarrierFv == 1) 
					F0 += GetRandom(-Fd, Fd);
				MF = settings.Acoustic3.ModF;
				Fd = settings.Acoustic3.ModFd;
				if (settings.Acoustic3.ModFv == 1) 
					MF += GetRandom(-Fd, Fd);
				MD = settings.Acoustic3.ModD;
				Fd = settings.Acoustic3.ModDd;
				if (settings.Acoustic3.ModDv == 1) 
					MD += GetRandom(-Fd, Fd);
				den = (double) settings.Acoustic3.Density / 10.0;
				Fd =  (double) settings.Acoustic3.Densityd;
				if (settings.Acoustic3.Densityv == 1)
					den += (GetRandom(-Fd, Fd) / 10.0);
				atten = settings.Acoustic3.Atten;
				Fd = settings.Acoustic3.Attend;
				if (settings.Acoustic3.Attenv == 1) 
					atten += GetRandom(-Fd, Fd);
			}
			fprintf(pFile,"%3d %4d %d %2d %4d %4d %4d %4.1f %3d\n",
				n,			// trial number
				tarR,		// target random
				ring,		// position target led
				spoke,
				F0,			// carrier frequency, F0
				MF,			// modulation freq.
				MD,			// modulation depth
				den,		// density
				atten		// attenuation
	...
	if (nTrials > 0)
		str.Format("Done %d trials",nTrials);
	else
		str.Format("Error, #trials = %d",nTrials);
	MessageBox(NULL,str,"Create Protocol",0);
}

void CMonkeyApp::OnProtocolRun()
{
	CProtRun Dlg;
	RECT rect1;
	RECT *pRect1 = &rect1;
	if (Dlg.DoModal() == IDOK)
	{
		pIO->m_IO_Prot = settings.Files.prot;
		pIO->UpdateData(false);
		pInfo->GetWindowRect(pRect1); 

		pCum->ShowWindow(SW_HIDE);
		pInfo->ShowWindow(SW_HIDE);
		pHisto->ShowWindow(SW_HIDE);

		pPassive->ShowWindow(SW_SHOW);
		pPassive->SetWindowPos(NULL,rect1.left,rect1.top,0,0,SWP_NOSIZE);
		execProtocol(); // change FSM and read header1 and text header2
	}
}
Protocol_Record1 *CMonkeyApp::getProtocolRecord1()
{
	return &Protocol_fixed;
}
Protocol_Record2 *CMonkeyApp::getProtocolRecord2()
{
	return &Protocol_vary;
}Protocol_Record2 *CMonkeyApp::getProtocolSaveRecord2()
{
	return &Protocol_save;
}
void CMonkeyApp::OnValveOpen()
{
	pFSM->openValve();
}

void CMonkeyApp::OnValveClose()
{
	pFSM->closeValve();
}

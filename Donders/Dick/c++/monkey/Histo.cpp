...
static  int	select = 4; // 1-visual, 2-auditory, 3-vis+aud, 4-total
// <-300,-300=>-250,-250=>-200,....,-50=>0,
//    0=>50,50=>100,....,950=>1000,>1000
static  int histoData1[29];	
static  int histoData2[29];	
static  int histoData3[29];	
static  int histoData4[29];	
...
BEGIN_MESSAGE_MAP(CHisto, CDialog)
	ON_BN_CLICKED(IDC_BUTTON1, &CHisto::OnBnClickedButton1)
END_MESSAGE_MAP()
...
void CHisto::DrawHisto()
{
	...
	{
	case 1:
		for (n=0; n<29; n++) histoData[n] = histoData1[n];
		break;
	case 2:
		for (n=0; n<29; n++) histoData[n] = histoData2[n];
		break;
	case 3:
		for (n=0; n<29; n++) histoData[n] = histoData3[n];
		break;
	case 4:
		for (n=0; n<29; n++) histoData[n] = histoData4[n];
		break;
	}
...
void CHisto::updateHisto(double fscore)
{
	...
	int kind = 0;
	if (pnt->getSettings()->Acoustic0.noSound) kind = 1;
	if (pnt->getSettings()->Parameters2.NoLed) kind = 2;
...
void CHisto::OnBnClickedButton1()
{
	select += 1;
	if (select > 4)
		select = 1;
	...
	{
	case 1:
		m_select.SetWindowTextA("Visual");
		break;
	case 2:
		m_select.SetWindowTextA("Auditory");
		break;
	case 3:
		m_select.SetWindowTextA("Vis+Aud");
		break;
	case 4:
		m_select.SetWindowTextA("Total");
		break;
	}
}

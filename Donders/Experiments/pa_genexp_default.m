...
int			= [45 55 65]; % this is approximately 40, 50 and 60 dBA, 3 levels
int			= repmat(int,m,1); % replicated by number of grid regions, so 
% we have mxn = 15x3 matrix, which equals the size of the target grid of every sound type
int			= int(:);
int			= repmat(int,3,1); % for bb, hp, lp types

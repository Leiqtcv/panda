function writeexp3(expfile,datdir,theta,phi,id,int,ledon,sndon,dummy)
...
%             fprintf(fid,'%s\t%d\t%d\t \t%d\t%d\t%d\t%d\t%d\n',...
%                 'LED',theta(i,j,k),phi(i,j,k),1,1,ledon(i,j,k)+sndon(i,j,k),1,ledon(i,j,k)+sndon(i,j,k)+150);
            if ~isempty(dummy)
                fprintf(fid,'\n');
                fprintf(fid,'%s\n','==>');

...
void mexFunction(int nLeft, mxArray *pLeft[],
	...
	if (nRight >= 2) {
        ...
        colLen  = mxGetM(xArg);
    	inpLen  = colLen*(rowLen+1);    // or strings
        arg     = mxCalloc(inpLen, sizeof(char)); // allocate space
    	mxGetString(xArg,arg,inpLen);   // arg points now to Arguments
        if (nRight == 3) {
            pR = mxGetPr(pRight[2]);
            timeOut = (int) *pR;
        }
		switch (mode) {
            ...
            case 3: //
                    cmd = (int)xValues[0];
                    Transmit(sprintf("$%d;%d\n",cmd,rowLen));
                    len = ReceiveLine();
                    if (len < 0) break;
                    for (n=1; n<rowLen; n++) {
                        Transmit(sprintf("$%d",(int) xValues[n]));
                        len = ReceiveLine();
                        if (len < 0) break;
                    }
                    break;
	...
	if (!leftFlag) {
		pLeft[0] = mxCreateDoubleMatrix(1,inpLen,mxREAL);
 	    pL = mxGetPr(pLeft[0]);
 		for (n=0; n<inpLen; n++) {
 			pL[n] = arg[n];
 		}

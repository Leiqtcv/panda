function varargout = pa_nirs_detect(varargin)
% PA_NIRS_DETECT M-file for pa_nirs_detect.fig
%      PA_NIRS_DETECT, by itself, creates a new PA_NIRS_DETECT or raises the existing
%      singleton*.
%
%      H = PA_NIRS_DETECT returns the handle to a new PA_NIRS_DETECT or the handle to
%      the existing singleton*.
%
%      PA_NIRS_DETECT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PA_NIRS_DETECT.M with the given input arguments.
%
%      PA_NIRS_DETECT('Property','Value',...) creates a new PA_NIRS_DETECT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before pa_nirs_detect_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to pa_nirs_detect_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help pa_nirs_detect

% Last Modified by GUIDE v2.5 27-Jul-2011 09:01:19

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
	'gui_Singleton',  gui_Singleton, ...
	'gui_OpeningFcn', @pa_nirs_detect_OpeningFcn, ...
	'gui_OutputFcn',  @pa_nirs_detect_OutputFcn, ...
	'gui_LayoutFcn',  [] , ...
	'gui_Callback',   []);
...
function pa_nirs_detect_OpeningFcn(hObject, eventdata, handles, varargin)
% --- Executes just before pa_nirs_detect is made visible.
home;
str = ['>>---- ' upper(mfilename) ' ----<<'];
disp(str);
...
% Choose default command line output for pa_nirs_detect
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);

function varargout = pa_nirs_detect_OutputFcn(hObject, eventdata, handles)
...
if lv<1
	handles.fname                               = [];
	handles.fname                               = fcheckexist(handles.fname,'*.xls');
end
if length(varargin)==1
	handles.fname                               = varargin{1};
	handles.fname                               = fcheckext(handles.fname,'.xls');
...
sdblock = nanstd(block,[],2)/sqrt(n);
x		= 1:length(mublock);
sel = ~isnan(mublock);
pa_errorpatch(x(sel),mublock(sel),sdblock(sel),'r');
hold on
xlabel('Time (samples)');
ylabel('OHb');
xlim([1 mndur+200])
...
x		= 1:length(mublock);
sel = ~isnan(mublock);
pa_errorpatch(x(sel),mublock(sel),sdblock(sel),'b');
xlabel('Time (samples)');
ylabel('OHb/DHb');
xlim([1 mndur+200])
% ylim(yax);
...
function h = pa_errorpatch(X,Y,E,markcol)
%  H = ERRORPATCH(X,Y,E)
%
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
fname        = fcheckext(handles.fname,'.mat');

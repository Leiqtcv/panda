...
for ii = 18:(nnmbr-5)
	try
		...
		for jj = 1:nfiles
			...
			S	= [S;nirs.signal(:,sel)]; %#ok<*AGROW>
			So	= [So;nirs.deepchan(:,sel)]; %#ok<*AGROW>
...
% 			keyboard
			...
			t = 1:length(x);
			t = t/fs;
			sel = t>onSample(1)/fd & t<offSample(1)/fd+10;
			x = x(sel,:);
			t = t(sel);
% 			x = detrend(x);
			...
			xlim([onSample(1)/fd-10 offSample(1)/fd+10]);
			ylim([-2 2]);
			
			pa_verline(onSample/fd);
			pa_verline(offSample/fd);
			pa_horline(0,'k:');

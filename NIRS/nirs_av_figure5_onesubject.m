function nirs_av_figure5_onesubject
...
for chanIdx = 1
	for modIdx	= 1:3
		...
		plot(x,nanmean(MU2),'-','Color',col{modIdx},'LineWidth',2)
		hold on
		% 				plot(x,MU2,'-','Color',col{modIdx},'LineWidth',1)
...
for chanIdx = 1
	...
	x		= x/10;
	subplot(121)
	plot(x-on(1)/10,y-offset,'k-')
	hold on
	% 	plot(x-on(1)/10,yfit,'k-','Color',[0.7 0.7 0.7]);
	box off
	axis square
end
...
xlim([-100 800])
ylim([-3 1]);
set(gca,'XTick',0:200:1000,'YTick',-2:0,'TickDir','out',...
	'TickLength',[0.005 0.025],...
	'FontSize',10,'FontAngle','Italic','FontName','Helvetica'); % publication-quality
xlabel('time re stimulus #1 (s)','FontSize',12);
ylabel('\Delta HbR (au)','FontSize',12);
title([]);
axis square;
box off
h = pa_text(0.05,0.9,'C');
set(h,'FontSize',15,'FontWeight','bold');
title('Subject 17, Right channel, block 1 (A, V, AV interleaved)','FontSize',12);
...
xlim([-5 50])
pa_errorpatch([10 30],[-6 -6],[20 20]);
set(gca,'XTick',0:10:40,'XTickLabel',(0:10:40)-10,'YTick',-2:0,'TickDir','out',...
	'TickLength',[0.005 0.025],...
	'FontSize',10,'FontAngle','Italic','FontName','Helvetica'); % publication-quality
xlabel('time re stimulus onset (s)','FontSize',12);
ylabel('Average \Delta HbR (au)','FontSize',12);
title([]);
axis square;
box off
h = pa_text(0.05,0.9,'D');
set(h,'FontSize',15,'FontWeight','bold');
title('Subject 17, Right channel, blocks 1-3 (A, V, AV interleaved)','FontSize',12);
...
%%
pa_datadir;
print('-depsc','-painters',[mfilename 'HbR']);

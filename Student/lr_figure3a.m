...
for jj = 1:4
    ...
    for ii = 1:3
        ...
        hold on
		mn = ((jj>2)-0.5)*0.3;
		mu = param(ii).chan(jj).mu-mn;
% 		mu = smooth(mu,50);
        h(ii) = pa_errorpatch(param(ii).chan(jj).time+(ii-1)*45,mu,param(ii).chan(jj).se,col(ii,:));
...
fs			= nirs.fsample;
fd			= nirs.fsdown;
onSample	= ceil([nirs.event.sample]); % onset and offset of stimulus
...
mx			= min((Aoff - Aon)+1)+3750;
nStim		= numel(Aon);
MU = NaN(nStim,mx);
for stmIdx = 1:nStim
    idx				= Aon(stmIdx)-2500:Aoff(stmIdx)+1250; % extra 100 samples before and after
    idx				= idx(1:mx);
    MU(stmIdx,:)	= chanSig(idx);
end
MU = bsxfun(@minus,MU,mean(MU(:,1:2500),2)); % remove the 100th sample, to set y-origin to stimulus onset

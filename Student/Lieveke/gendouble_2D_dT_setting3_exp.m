...
expfile = 'double_dt_2d_setting4.exp';
Fs          = 48828.125;
...
Psingle = p;
Psingle = Psingle(:);
Tsingle = repmat(-30,size(Psingle));
...
%% Write
marc
writeexp4(expfile,datdir,T,P1,P2,dT,Tsingle,Psingle);
%% Show the exp-file in Wordpad
if ispc && showexp
	dos(['"C:\Program Files\Windows NT\Accessories\wordpad.exe" ' expfile ' &']);
end

function writeexp4(expfile,datdir,T,P1,P2,dT,Tsingle,Psingle)
...
for i               = 1:ntrials2		% each location
	...
	Psingle(i)
	writesnd(fid,'SND1',Tsingle(i),Psingle(i),300+Psingle(i),int,1,ledon+100); % Sound on

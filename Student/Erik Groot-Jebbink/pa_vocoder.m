function pa_vocoder(wave,Fs)
% PA_VOCODER(WAVE,FS)
%


% 2012 Erik-Groot Jebbink, Marc van Wanrooij
...
cd(d);
if nargin<1
		[~,Fs]	= wavread('test_wav_kort.wav'); %laad bestand
	[wave,fs] = pa_genripple(8,1,20,500,500,'display',666);
	wave = wave';
% 	whos wave
end
% return
...
if ruis_sinus == 0 % moduleer op ruis
	...
	for ii=1:kanalen,
		subplot(kanalen/2,2,ii);
		plot(t,simulatie_cis(ii,:),'g-','LineWidth',2);
	end
drawnow;
end	
	% sommeer de banden

function [samples, stats, structArray] = pa_matjags(dataStruct, jagsFilenm, initStructs , varargin)
...
codaF					= [ 'CODA' , num2str(1) , 'chain1.txt' ];
S						= bugs2mat(codaIndex, codaF);
structArray(1:nChains)	= S;
for i				= 1:nChains
...
for i=1:Nparam
	...
	elseif ((length(size(val)) == 2) && ((sfield1 == 1) || (sfield2 == 1)))
		...
		elseif length(valsize)==3
			clear valTransp
			if dotranspose==1
				valTransp = permute(val,[3 1 2]); % Does this do the same thing?
% 				for j=1:valsize(3)
...
% 				end
			...
			disp('Error: 4D and higher dimensional arrays not accepted');
			return
		end
...
for fi=1:length(fld)
	...
	if (length(st_mean_size) == 2) && (st_mean_size(2) == 1)
		...
		stats.std.(fname) = st_std_overall;
end
end
...
for k=1:Nvars
	...
	if size(indices)~=[1 2] %#ok<BDSCA>
		error(['Cannot read line: [' ind(k,:) ']']);
	end
% 	sdata = size(data);
	%indices
	samples=data(indices(1):indices(2),2); %#ok<NASGU>
	varname(varname=='[')='(';
	varname(varname==']')=')';
